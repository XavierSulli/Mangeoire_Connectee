...
void beforeinitstate(void);
void initstate(void);
void takepicstate(void);
void gpsstate(void);
...
int main()
{
	...
	my_printf("\r\n");
	my_printf("---------------------------------------\r\n");
	my_printf("-           M4NG3O1RE SEXY            -\r\n");
	my_printf("-  Polytech MEA -  Montpellier 2020   -\r\n");
	my_printf("- Firmware version 1.0.0 (13/10/2020) -\r\n");
	my_printf("---------------------------------------\r\n");
	my_printf("\r\n");
...
void takepicstate(void)
{
	...
	delay_ms(800);  	// Wait for 800ms -> cam automatic gain cell
	...
	while ((ADC2->ISR & ADC_ISR_ADRDY) != ADC_ISR_ADRDY); // the code stops here
	...
	BSP_PA1_Off();
	BSP_PC3_Off();
	if(debug) my_printf("---------------------------------------------> Entering Default State\r\n");
...
void gpsstate(void)
{
	...
	uint8_t		heading_dec = 0;		// 00 to 99 (2 decimals)
	...
	speed_dec = heading_dec;
	heading_dec = speed_dec;

	// Start GPS Stream
	uart_gps_init();
	DELAY_TIM_ms(5000);// if not us then ms
	BSP_PC2_Init();
	BSP_PA9_Init();
	BSP_PA9_On();
...
void blestate(void)
{
	...
	DELAY_TIM2_init();
	NVIC_SetPriority(TIM2_IRQn, 5);//5
